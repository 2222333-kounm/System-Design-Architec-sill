...
>
> **📁 当前目录为项目产出子目录，根目录 `../README.md` 有完整项目结构说明。**
...
本项目包含一个 **DESIGN.md → Code** 自动提取脚本，将设计规范与实现代码自动同步：

```bash
# 查看 Usage
node scripts/extract-tokens.js

# 解析 DESIGN.md → tokens.json (Machine-readable)
node scripts/extract-tokens.js parse
...
DESIGN.md (Single Source of Truth)
    │
    ├─ node scripts/extract-tokens.js parse
    │   └─ css/tokens.json (Machine-readable, 供工具链消费)
    │
    ├─ node scripts/extract-tokens.js css
